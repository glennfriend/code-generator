...
{/if}{/foreach}
    ...
    public function add{$oName3}( $object )
    {
        ...
        $object = $this->get{$oName3}( $insertId );
        if ( !$object ) {
            return false;
        }
    ...
    protected function _getByFind{$mName3}Option( $opt=array(), $isGetCount=false )
    {
        ...
        if ( !$isGetCount ) {
            // $limit = BaseModelHelper::getLimit( $opt['page'], $opt['itemsPerPage'] );
            $condition = array(
    ...
    /**
     *  find{$mName3} option by SQL query 
     *  @return objects or record total
     */
    protected function xxxxxxxxxx( $opt=array(), $isGetCount=false )
    {
        // 大致同上, 略....
        xxxxxxxxxx
        xxxxxxxxxx
        ...
        $whereString = '';
        if ( $where ) {
            $whereString = ' WHERE '. trim(join(' AND ',$where));
        }

        // order
        $orderString = '';
        if ( $opt['order'] ) {
            $orderString = ' ORDER BY '. $opt['order'];
        }

        ${$mName2}Record = new {$mName3}Record();

        if ( !$isGetCount ) {
            $limit = BaseModelHelper::getLimit( $opt['page'], $opt['itemsPerPage'] );
            $sql = "
                SELECT id
                FROM {ldelim}$articlesRecord->tableName(){rdelim}
                {ldelim}$whereString{rdelim}
                {ldelim}$orderString{rdelim}
                {ldelim}$limit{rdelim}
            ";
            ...
            foreach ( $results as $result ) {
                if ( ${$oName2} = ${$mName2}->get{$oName3}( $result['id'] ) ) {
                    $objects[] = ${$oName2};
                }
            }
            return $objects;
